...
# 定义常量
MOVIE_LIST_FILE = "resources/movie_list2.csv"
...
def get_movie_year(movie_year=None):
    movie_year = movie_year[0].strip() if movie_year else ""
    return movie_year.replace("(", "").replace(")", "").strip()


def get_release_date(movie_release_date=None):
    release_date = movie_release_date[0].strip() if movie_release_date else ""
    return re.search(r"\d{4}-\d{2}-\d{2}", release_date).group()


def get_movie_time(movie_time=None):
    cost_time = movie_time[0].strip() if movie_time else ""
    result = re.search(r"(\d+)h?(\d+)m?", cost_time)
    hours = int(result.group(1)) if result else ""
    minutes = int(result.group(2)) if result else ""
...
def get_movie_info(movie_info_url):
    ...
    return {
        ...
        "上映时间": get_release_date(movie_release_date),
